...
const RankingTab = ({ students }: { students: any[] }) => {
    ...
    const filteredData = useMemo(() => {
        ...
        // 2. Filter by School & Kecamatan
        res = res.filter(d => {
            const userKecamatan = userMap[d.username]?.kecamatan || '-';
    ...
    }, [data, filterKecamatan, filterSchool, filterSubject, userMap]);
            ...
            <div className="overflow-x-auto rounded-lg border border-slate-200">
                <table className="w-full text-xs md:text-sm text-left">
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {loading ? (
                            <tr><td colSpan={7} className="p-8 text-center text-slate-400"><Loader2 className="animate-spin inline mr-2"/> Memuat data ranking...</td></tr>
                        ) : filteredData.length === 0 ? (
                            <tr><td colSpan={7} className="p-8 text-center text-slate-400 italic">Data tidak ditemukan untuk mapel ini.</td></tr>
                        ) : (
